...
DynamixelController::DynamixelController() : Node("dynamixel_workbench_controllers"),
  ...
  // jnt_tra_msg_ is no longer a pointer in our header for better memory safety
  ...
  DynamixelController::~DynamixelController() 
{
  ...
  // delete dxl_wb_;
  // delete jnt_tra_;
  // unique_ptr cleans itself automatically.
}
...
bool DynamixelController::initDynamixels(void)
{
  ...
  for (auto const& dxl:dynamixel_)
  {
    ...
    const char* log;
    dxl_wb_->readRegister((uint8_t)dxl.second, "Torque_Enable", &torque_val, &log);
...
bool DynamixelController::getPresentPosition(std::vector<std::string> dxl_name)
{
  ...
  for (auto const& name : dxl_name)
    RCLCPP_INFO(this->get_logger(), "  joint requested: %s -> ID %d", name.c_str(), dynamixel_[name]);
  ...
  if (dxl_wb_->getProtocolVersion() == 2.0f)
  {
    RCLCPP_INFO(this->get_logger(), "Protocol 2.0 — calling syncRead");
    ...
    {
      RCLCPP_INFO(this->get_logger(), "syncRead OK");
    }
  ...
  {
    RCLCPP_INFO(this->get_logger(), "Protocol 1.0 — reading individually");
    WayPoint wp;
  ...
  RCLCPP_DEBUG(this->get_logger(), "getPresentPosition done — pre_goal size=%ld result=%d",
              pre_goal_.size(), result);
  return result;
}
...
void DynamixelController::initPublisher()
{
  ...
  dynamixel_state_list_pub_ = this ->create_publisher<dynamixel_workbench_msgs::msg::DynamixelStateList>("dynamixel_state",10);
...
void DynamixelController::initSubscriber()
{
  ...
  trajectory_sub_ = this->create_subscription<trajectory_msgs::msg::JointTrajectory>("joint_trajectory",10,std::bind(&DynamixelController::trajectoryMsgCallback,this,std::placeholders::_1));
  if (is_cmd_vel_topic_){
    cmd_vel_sub_ = this->create_subscription<geometry_msgs::msg::Twist>("cmd_vel",10,std::bind(&DynamixelController::commandVelocityCallback,this,std::placeholders::_1));
  }
}
...
void DynamixelController::readCallback()
{
    ...
    std::lock_guard<std::mutex> lock(dxl_mutex_);//feature

#ifdef DEBUG
  static double priv_read_secs =this->get_clock()->now().seconds();
#endif
  bool result = false;
  const char* log = NULL;
  ...
  // vla arrays are non standard in C++ 
...
#ifdef DEBUG
  RCLCPP_WARN(this->get_logger(),"[readCallback] diff_secs : %f", this->get_clock()->now().seconds() - priv_read_secs);
  priv_read_secs = this->get_clock()->now().seconds();
#endif
}
...
#ifdef DEBUG
  static double priv_pub_secs =this->get_clock()->now().seconds();
#endif
if (dynamixel_state_list_.dynamixel_state.size() != dynamixel_.size()) return;
  ...
  if (is_joint_state_topic_)
  {
    ...
    //RCLCPP_WARN(this->get_logger(),"dxl map size=%ld  state size=%ld", dynamixel_.size(), dynamixel_state_list_.dynamixel_state.size()); //for debugging seg fault
    joint_states_pub_->publish(joint_state_msg_);
  }

#ifdef DEBUG
  RCLCPP_WARN(this->get_logger(),"[publishCallback] diff_secs : %f", this->get_clock()->now().seconds() - priv_pub_secs);
  priv_pub_secs = this->get_clock()->now().seconds();
#endif
}
  ...
  std::shared_ptr<const geometry_msgs::msg::Twist> msg)
{
  ...
  std::vector<double> wheel_velocity(dynamixel_.size()); // VLA
  std::vector<int32_t> dynamixel_velocity(dynamixel_.size()); // VLA

  const uint8_t LEFT = 0;
  const uint8_t RIGHT = 1;

  double robot_lin_vel = msg->linear.x; // if no shared ownership , msg.linear.x
  double robot_ang_vel = msg->angular.z;

  std::vector<uint8_t> id_array(dynamixel_.size()); // VLA
  uint8_t id_cnt = 0;
  ...
  if (is_moving_ == true)
  {
    for (uint8_t index = 0; index < id_cnt; index++)
    {
      ...
      RCLCPP_DEBUG(this->get_logger(), "writeCallback: ID=%d raw_value=%d from_radian=%.4f",
            ...
            jnt_tra_msg_.points[point_cnt].positions.at(index));  // ADD THIS
    }

    result = dxl_wb_->syncWrite(SYNC_WRITE_HANDLER_FOR_GOAL_POSITION, id_array.data(), id_cnt, dynamixel_position.data(), 1, &log);
    RCLCPP_DEBUG(this->get_logger(), "syncWrite result=%d point=%d/%ld",result, point_cnt, jnt_tra_msg_.points.size());  // ADD THIS
    
    // TEMP: replace syncWrite with itemWrite
// result = dxl_wb_->itemWrite((uint8_t)id_array[0], "Goal_Position", dynamixel_position[0], &log);
// RCLCPP_INFO(this->get_logger(), "itemWrite result=%d val=%d", result, dynamixel_position[0]);
...
#ifdef DEBUG
  RCLCPP_WARN("[writeCallback] diff_secs : %f", this->get_clock()->now().seconds() - priv_pub_secs);
  priv_pub_secs = this->get_clock()->now().seconds();
#endif
}
...
RCLCPP_INFO(this->get_logger(), "move_time = %.4f sec", rclcpp::Duration(msg->points[0].time_from_start).seconds());
//REMOVE AFTER DEBUG
    ...
    if (id_cnt != 0)
    {
      ...
      while(cnt < msg->points.size())
      {
        ...
        else
        {
          ...
          if (cnt == 0) move_time = rclcpp::Duration(msg->points[cnt].time_from_start).seconds();
          else move_time = rclcpp::Duration(msg->points[cnt].time_from_start).seconds() - rclcpp::Duration(msg->points[cnt-1].time_from_start).seconds();

          // todo - > DONE
          // Wrong:
          //msg->points[cnt].time_from_start.seconds()
          // Fix:
          //rclcpp::Duration(msg->points[cnt].time_from_start).seconds()
          // Both occurrences on lines 736-737
...
bool DynamixelController::dynamixelCommandMsgCallback(const std::shared_ptr<dynamixel_workbench_msgs::srv::DynamixelCommand::Request> req, std::shared_ptr<dynamixel_workbench_msgs::srv::DynamixelCommand::Response> res)
{
  ...
  uint8_t id = req->id; //uint8_t id = req.id;
  std::string item_name = req->addr_name; //std::string item_name = req.addr_name;
  int32_t value = req->value; //int32_t value = req.value;
...
//######################### todo : identify the .data() for vectors
...
int main(int argc, char **argv)
{
  ...
  if (argc < 2)
  {
    RCLCPP_ERROR(dynamixel_controller->get_logger(),"Please set '-port_name' and  '-baud_rate' arguments for connected Dynamixels");
    return 0;
  }
  else
  {
  ...
  dynamixel_controller->get_parameter("dynamixel_info",yaml_file);


  result = dynamixel_controller->initWorkbench(port_name, baud_rate);
  if (result == false)
  {
    RCLCPP_ERROR(dynamixel_controller->get_logger(),"Please check USB port name");
    return 0;
  }
  ...
  if (result == false)
  {
    RCLCPP_ERROR(dynamixel_controller->get_logger(),"Please check YAML file");
    return 0;
  }
  ...
  if (result == false)
  {
    RCLCPP_ERROR(dynamixel_controller->get_logger(),"Please check Dynamixel ID or BaudRate");
    return 0;
  }
  ...
  if (result == false)
  {
    RCLCPP_ERROR(dynamixel_controller->get_logger(),"Please check control table (http://emanual.robotis.com/#control-table)");
    return 0;
  }
  ...
  if (result == false)
  {
    RCLCPP_ERROR(dynamixel_controller->get_logger(),"Please check control items");
    return 0;
  }
  ...
  if (result == false)
  {
    RCLCPP_ERROR(dynamixel_controller->get_logger(),"Failed to set Dynamixel SDK Handler");
    return 0;
  }
  ...
  // //create timer for read, write and publish
  // auto read_timer = dynamixel_controller->create_wall_timer(std::chrono::duration<double>(dynamixel_controller->getReadPeriod()),[dynamixel_controller]() { dynamixel_controller->readCallback();});
  // auto write_timer = dynamixel_controller->create_wall_timer(std::chrono::duration<double>(dynamixel_controller->getWritePeriod()),[dynamixel_controller]() { dynamixel_controller->writeCallback();});
  // auto publish_timer = dynamixel_controller->create_wall_timer(std::chrono::duration<double>(dynamixel_controller->getPublishPeriod()),[dynamixel_controller]() { dynamixel_controller->publishCallback();});

    auto read_timer = dynamixel_controller->create_timer(std::chrono::duration<double>(dynamixel_controller->getReadPeriod()),[dynamixel_controller]() { dynamixel_controller->readCallback();});
  auto write_timer = dynamixel_controller->create_timer(std::chrono::duration<double>(dynamixel_controller->getWritePeriod()),[dynamixel_controller]() { dynamixel_controller->writeCallback();});
  auto publish_timer = dynamixel_controller->create_timer(std::chrono::duration<double>(dynamixel_controller->getPublishPeriod()),[dynamixel_controller]() { dynamixel_controller->publishCallback();});
  
  


  RCLCPP_INFO(dynamixel_controller->get_logger(), "Dynamixel controller node started");
